...
> **注**: このドキュメントは要件定義 → 設計フェーズの一時的な橋渡し文書です。設計フェーズ完了後は `docs/design/` 配下のドキュメントが最新の仕様を表します。歴史的経緯としてアーカイブ保管しています。
...
| ファイル | 内容 |
|---------|------|
| `docs/requirements/01-functional-requirements.md` | 機能要件定義（F1〜F5 + 将来対応） |
| `docs/requirements/02-technical-requirements.md` | 技術要件定義（システム構成・インフラ・非機能要件・技術スタック） |
