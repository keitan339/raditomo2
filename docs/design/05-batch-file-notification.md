...
要件 F3 「番組名ごとにグルーピングして表示する」を満たすため、フォルダ名には**番組タイトルそのものではなく「グループキー」**を使う。グループキーは番組タイトルから「○時台」「(ZERO)」「(N)」などの**回ごとに変動する接尾辞を除去**した共通名。

| 番組タイトル | グループキー（フォルダ名） | ファイル名 |
|---|---|---|
| `らじらー！　サンデー　８時台　…` | `らじらー！　サンデー` | `らじらー！　サンデー　８時台　…_20260426-2005.mp3` |
| `らじらー！　サンデー　９時台　…` | `らじらー！　サンデー` | `らじらー！　サンデー　９時台　…_20260426-2105.mp3` |
| `佐久間宣行のオールナイトニッポン0(ZERO)` | `佐久間宣行のオールナイトニッポン0` | `佐久間宣行のオールナイトニッポン0(ZERO)_20260430-0300.mp3` |

これにより、F3 画面の「番組ごとに各回が並ぶ」表示と、ディスク上のフォルダ単位の整理が一致する。グループ化規則（正規表現）は `RecordingTitleGrouper` で集中管理する。

ID3 Album タグには「番組グループ名」ではなく**素の番組名**（要件 F2 通り）を入れる。フォルダ名と Album タグはこの 1 点で乖離する。
